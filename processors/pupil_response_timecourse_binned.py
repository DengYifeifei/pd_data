...
def process_folder(folder):
    ...
    # )
    # response_pupils = np.load(load_path, allow_pickle=True)

    save_path = os.path.join(
        eye_directory, folder, "pupil_response_binned.npy"
    )
    ...
    for i in trial_index_list:
        ...
        t_start_vals = participant.eye_raw[
            ...
            (participant.eye_raw['Event'] == 'response')
        ]['TimeEvent'].values
        ...
        response_eye_data = trial_eye_data[trial_eye_data['TimeEvent'] > t_start]

        # -------------------------------
        # Extract and clean data
        # -------------------------------
        times = response_eye_data['TimeEvent'].values - t_start
        pupils = response_eye_data['Pupil'].values
